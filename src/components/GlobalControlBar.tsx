"use client";

import { useState, useEffect, useRef } from "react";

interface GlobalControlBarProps {
  lang?: "ru" | "en";
  setLang?: (lang: "ru" | "en") => void;
}

export default function GlobalControlBar({
  lang = "ru",
  setLang
}: GlobalControlBarProps) {
  const [isPlaying, setIsPlaying] = useState<boolean>(false);
  const [currentLang, setCurrentLang] = useState<"ru" | "en">(lang);
  const audioRef = useRef<HTMLAudioElement | null>(null);

  useEffect(() => {
    setCurrentLang(lang);
  }, [lang]);

  useEffect(() => {
    // Check saved audio preference
    const savedAudioPref = localStorage.getItem("kommunarka_ambient_audio");
    if (savedAudioPref === "true" && audioRef.current) {
      audioRef.current
        .play()
        .then(() => setIsPlaying(true))
        .catch(() => setIsPlaying(false));
    }
  }, []);

  const toggleAudio = () => {
    if (!audioRef.current) return;

    if (isPlaying) {
      audioRef.current.pause();
      setIsPlaying(false);
      localStorage.setItem("kommunarka_ambient_audio", "false");
    } else {
      audioRef.current
        .play()
        .then(() => {
          setIsPlaying(true);
          localStorage.setItem("kommunarka_ambient_audio", "true");
        })
        .catch((err) => {
          console.log("Audio play blocked by browser policy:", err);
        });
    }
  };

  const handleLangChange = (newLang: "ru" | "en") => {
    setCurrentLang(newLang);
    if (setLang) {
      setLang(newLang);
    }
  };

  return (
    <div className="fixed top-20 md:top-24 right-4 md:right-12 z-50 flex items-center border border-white/15 p-1 bg-black/85 backdrop-blur-md text-xs font-mono shadow-2xl">
      <audio
        ref={audioRef}
        src="/assets/ambient-ritual.mp3"
        loop
        preload="auto"
      />

      {/* Sound Button */}
      <button
        onClick={toggleAudio}
        className={`px-2.5 py-1 transition-all flex items-center gap-1.5 border-r border-white/15 ${
          isPlaying
            ? "text-accent font-bold bg-accent/10"
            : "text-zinc-400 hover:text-white"
        }`}
        title={isPlaying ? "Mute Sound" : "Play Sound"}
      >
        <span className="flex items-center gap-1">
          <span className={`w-1.5 h-1.5 rounded-full ${isPlaying ? "bg-accent animate-ping" : "bg-zinc-600"}`} />
          {isPlaying ? (
            <svg className="w-3.5 h-3.5 fill-current text-accent" viewBox="0 0 24 24">
              <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/>
            </svg>
          ) : (
            <svg className="w-3.5 h-3.5 fill-current text-zinc-500" viewBox="0 0 24 24">
              <path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z"/>
            </svg>
          )}
        </span>
        {/* Full Text on Desktop, Icon-only on Mobile */}
        <span className="hidden sm:inline tracking-wider">
          {isPlaying
            ? currentLang === "ru" ? "ЗВУК" : "SOUND"
            : currentLang === "ru" ? "ЗВУК" : "SOUND"}
        </span>
      </button>

      {/* Language Switcher Buttons */}
      <div className="flex items-center pl-1 gap-1">
        <button
          onClick={() => handleLangChange("ru")}
          className={`px-2.5 py-1 transition-all ${
            currentLang === "ru" ? "bg-accent text-black font-bold" : "text-zinc-400 hover:text-white"
          }`}
        >
          RU
        </button>
        <button
          onClick={() => handleLangChange("en")}
          className={`px-2.5 py-1 transition-all ${
            currentLang === "en" ? "bg-accent text-black font-bold" : "text-zinc-400 hover:text-white"
          }`}
        >
          EN
        </button>
      </div>
    </div>
  );
}
